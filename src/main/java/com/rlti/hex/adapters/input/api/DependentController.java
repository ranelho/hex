...
public class DependentController {
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Tipos de dependentes encontrados com sucesso",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = DependentResponse.class))
            ),
            @ApiResponse(
                    responseCode = "404",
                    description = "Nenhum tipo de dependente encontrado",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = Object.class))
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "Erro ao buscar tipos de dependentes",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = Object.class))
            )
    })
    @GetMapping("/dependent-type")
    public ResponseEntity<List<String>> findDependentType() {
        List<String> dependentTypes = Stream.of(DependentType.values())
                .map(DependentType::getDescription)
                .toList();
        return ResponseEntity.ok(dependentTypes);
    }
